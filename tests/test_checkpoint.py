...
def _make_minimal_trainer(tmp_path: Path, *, epochs: int = 10) -> Trainer:
    trainer = Trainer.__new__(Trainer)
    trainer.config = {"train": {"epochs": epochs}}
    trainer.device = torch.device("cpu")
    trainer.model = nn.Linear(2, 1)
    trainer.optimizer = torch.optim.AdamW(trainer.model.parameters(), lr=1.0e-3)
